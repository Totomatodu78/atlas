...
function errorText(err) {
  ...
  if (/already registered|already been registered|already exists/i.test(msg)) return 'Un compte existe déjà avec cette adresse. Utilise « Se connecter ».';
  ...
  if (/password/i.test(msg) && /should|weak|least|characters|known/i.test(msg)) return `Mot de passe trop faible : ${MIN_PASSWORD} caractères minimum.`;
...
// ---------------------------------------------------------------------------
// Connexion par e-mail et mot de passe (aucun e-mail n'est envoyé)
// ---------------------------------------------------------------------------
...
function selectAuthTab(name) {
  ...
  $(name === 'login' ? '#login-email' : '#signup-email').focus();
}
...
$('#form-signup').addEventListener('submit', async (e) => {
  ...
  if (error) return toast(errorText(error), 'error');
  if (!data.session) {
    // Supabase attend encore une confirmation par e-mail : réglage à désactiver
    return toast('Compte créé, mais Supabase demande une confirmation par e-mail. Désactive « Confirm email » dans Supabase, puis connecte-toi.', 'error');
  }
...
$('#form-password').addEventListener('submit', async (e) => {
  ...
  if (password.length < MIN_PASSWORD) return toast(`Le mot de passe doit faire au moins ${MIN_PASSWORD} caractères.`, 'error');
  if (password !== $('#new-password2').value) return toast('Les deux mots de passe ne sont pas identiques.', 'error');
  const btn = e.submitter || e.target.querySelector('[type=submit]');
